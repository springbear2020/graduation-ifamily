  ...
  <div>
    ...
    <van-form @submit="handleSubmit">
      ...
      <van-field label="家族地址" readonly clickable name="area" size="large" placeholder="点击选择省市区" required
                 ...
                 v-model="formData.address" @click="showAreaPopup = true; areaType = '0'"/>
      <van-field label="家族祖籍" readonly clickable name="area" size="large" placeholder="点击选择省市区"
                 v-model="formData.ancestryAddress" @click="showAreaPopup = true; areaType = '1'"/>

      <!-- 简介、字辈歌 -->
      <van-field rows="2" autosize label="家族简介" type="textarea" maxlength="100" size="large" placeholder="家族简介"
                 v-model="formData.introduction" show-word-limit clearable/>
      <van-field rows="2" autosize label="字辈歌" type="textarea" maxlength="100" size="large" placeholder="字辈歌"
                 v-model="formData.generationSong" show-word-limit clearable/>

      <div class="block-button-container">
        <van-button type="info" block>立即创建</van-button>
      </div>
    </van-form>
...
export default {
  ...
  computed: {
    title() {
      return this.type === '0' ? '创建家族' : (this.type === '1' ? '编辑家族' : '')
    },
    dstRoute() {
      return this.type === '0' ? '/family' : (this.type === '1' ? '/family/manage' : '/')
    }
  },
  methods: {
    confirmArea(area) {
